...
- Added a py.typed file that allows other projects that pip install heather-blockchain to type check using our functions with mypy.
...
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Chia-Network/heather-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.
...
- The `src` directory in heather-blockchain has been changed to `chia` to avoid namespace collisions.
...
- You should review the [release notes for v1.0.2](https://github.com/Chia-Network/heather-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/heather-blockchain).
...
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/heather-blockchain/).
...
- Users can now pip install e.g. heather-blockchain==1.0.1 on most platforms.
...
- Migration with HEATHER_ROOT set does not crash chia init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. THEATHER Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.chia.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `chia farm summary` will now display THEATHER or HEATHER as appropriate.
...
- The Farm page will now no longer get stuck at 50 THEATHER farmed.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `heather_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of THEATHER will not come forward.
...
- RC3 is a new chain to support the last major chialisp changes. THEATHER from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'chia wallet send' now takes in THEATHER or HEATHER as units instead of ch.
...
- The Beta 27 chain is a hard fork. All THEATHER from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 THEATHER/HEATHER per block for a total of 9216 THEATHER per day from 4608 challenges per day.
...
- The cli wallet, `chia wallet`, was incorrectly displaying THEATHER balances as if they were Coloured Coins.
...
- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 THEATHER as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/Chia-Network/clvm_rs) is complete.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 THEATHER. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 THEATHER compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 THEATHER.
...
- New consensus means this is a very hard fork. All of your THEATHER from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 THEATHER to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 THEATHER to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much THEATHER you've won so far. Harvester and Full Node connections have moved to Advanced Options.
...
- Changed HEATHER units to THEATHER units for testnet.
...
- The user interface now refers to chia coins with their correct currency code of HEATHER.
...
- Configuration directories have been moved to a default location of HomeDirectory/.chia/release/config, plots/ db/, wallet/ etc. This can be overridden by `export HEATHER_ROOT=~/.chia` for example which would then put the plots directory in `HomeDirectory/.chia/plots`.
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:8014/index.html](http://127.0.0.1:8014/index.html) with any modern web browser on the same machine as the full node.
...
- Implemented an RPC interface with JSON serialization for streamables - currently on port 8014.
