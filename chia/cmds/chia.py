...
@click.group(
    help=f"\n  Manage heather blockchain infrastructure ({__version__})\n",
    epilog="Try 'heather start node', 'heather netspace -d 192', or 'heather show -s'",
...
@cli.command("version", short_help="Show heather version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs heather daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by heather start <service>
)
@click.pass_context
