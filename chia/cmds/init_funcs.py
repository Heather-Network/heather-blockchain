...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    all_targets = []
    stop_searching_for_farmer = "heather_target_address" not in config["farmer"]
    stop_searching_for_pool = "heather_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("heather_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("heather_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "heather_target_address" not in config["farmer"]:
        print(
            f"Setting the heather destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["heather_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["heather_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['heather_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "heather_target_address" not in config["pool"]:
        print(f"Setting the heather destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["heather_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["heather_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['heather_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
            f"To change the HEATHER destination addresses, edit the `heather_target_address` entries in"
...
def create_all_ssl(root_path: Path):
    ...
    chia_ca_crt_path = ca_dir / "heather_ca.crt"
    chia_ca_key_path = ca_dir / "heather_ca.key"
...
def chia_init(
    ...
    """
    if os.environ.get("HEATHER_ROOT", None) is not None:
        print(
            f"warning, your HEATHER_ROOT is set to {os.environ['HEATHER_ROOT']}. "
    ...
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if HEATHER_ROOT is set, or if user has run chia init twice
        # before a new update.
        if testnet:
