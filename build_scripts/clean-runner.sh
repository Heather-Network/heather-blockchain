...
rm -rf heather-blockchain-gui/build || true
rm -rf heather-blockchain-gui/daemon || true
rm -rf heather-blockchain-gui/node_modules || true
rm heather-blockchain-gui/temp.json || true
( cd "$PWD/heather-blockchain-gui" && git checkout HEAD -- package-lock.json ) || true
cd "$PWD" || true
