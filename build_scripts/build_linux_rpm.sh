...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="heather-blockchain-linux-x64"
else
	#PLATFORM="$1"
	DIR_NAME="heather-blockchain-linux-arm64"
fi
...
HEATHER_INSTALLER_VERSION=$(python installer-version.py)

if [ ! "$HEATHER_INSTALLER_VERSION" ]; then
...
# sets the version for heather-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$HEATHER_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json

electron-packager . heather-blockchain --asar.unpack="**/daemon/**" --platform=linux \
--icon=src/assets/img/Chia.icns --overwrite --app-bundle-id=net.chia.blockchain \
--appVersion=$HEATHER_INSTALLER_VERSION
LAST_EXIT_CODE=$?
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create heather-blockchain-$HEATHER_INSTALLER_VERSION.rpm"

	# shellcheck disable=SC2046
	NODE_ROOT="$(dirname $(dirname $(which node)))"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Chia-Network/heather-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $HEATHER_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
